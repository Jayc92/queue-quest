...
import { CONFIGURATION_TOUR } from '../data/tours';
...
export function ConfigurationScreen({ level, initialConfig, onRunSimulation, onBack }: Props) {
    ...
    // Live projections run against the scenario-adjusted level so the HUD,
    // traffic viz, and warnings reflect the deterministic pre-launch event.
    const effectiveLevel = useMemo(() => applyScenario(level), [level]);
    const projections = useMemo(() => calculateProjections(effectiveLevel, config), [effectiveLevel, config]);
    const prevProjRef = useRef(projections);

    // Signal queue-open once when the console mounts (no-op audio sink today).
    useEffect(() => { playSound('queue_open'); }, []);

    // Danger-severity warnings drive launch-button intensity.
    const dangerCount = projections.warnings.filter(w => w.severity === 'danger').length;
    ...
    useEffect(() => {
        ...
        // A newly-raised danger warning fires the warning sound hook.
        const prevDanger = prev.warnings.filter(w => w.severity === 'danger').length;
        const nextDanger = next.warnings.filter(w => w.severity === 'danger').length;
        if (nextDanger > prevDanger) playSound('warning');
    ...
    const ProjectionsContent = () => (
        <div className="space-y-3">
            ...
            <div data-tour="config-alerts">
                ...
                <div className="pt-2 mt-3 border-t border-slate-800">
        ...
        <div className="min-h-screen grid-bg pb-32 lg:pb-6">
            <div className="sticky top-0 z-30 bg-terminal-bg/95 backdrop-blur border-b border-slate-800 px-3 py-2">
                <div className="max-w-6xl mx-auto flex items-center justify-between gap-2">
                    <button onClick={onBack} aria-label="Back to briefing" className="flex items-center gap-1.5 text-slate-400 hover:text-white py-1 px-1 shrink-0">
            ...
            <div className="max-w-6xl mx-auto p-3 md:p-4">
                <PressureHUD level={effectiveLevel} projections={projections} config={config} />
                <QueueTraffic level={effectiveLevel} projections={projections} config={config} />

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4">
                    <div className="lg:col-span-2 space-y-3 md:space-y-4">
                        <div data-tour="config-timing">
                        <ConsolePanel title="Queue Timing" icon="Clock"
                            ...
                            <div className="space-y-5">
                                <GameSlider label="Waiting Room Opens" value={config.waitingRoomTime}
                                    ...
                                    helpText="Hours before doors open. Longer gives fans time to arrive — but also gives bots more time to prepare." />
                                <GameSlider label="Entry Wave Count" value={config.waveCount}
                                    onChange={(v) => updateConfig('waveCount', v)} min={1} max={8}
                                    helpText="Splits the crowd into batches so the server isn't hit all at once. 2–4 protects stability; 5+ over-stresses it." />
                                {config.waveCount > 1 && (
                                    <GameSlider label="Wave Interval" value={config.waveInterval}
                                        ...
                                        helpText="Minutes between each batch. Longer gaps ease server load; shorter gaps make fans wait less." />
                                )}
                            </div>
                        </ConsolePanel>
                        </div>

                        <div data-tour="config-defense">
                        <ConsolePanel title="Identity & Bot Defense" icon="Shield"
                            ...
                            <div className="space-y-4">
                                <SegmentedControl label="Bot Detection" options={BOT_DETECTION_OPTIONS}
                                    ...
                                    helpText="Stronger settings block more bots but add friction that can slow real fans. Match it to the mission's bot threat." />
                                <SegmentedControl label="Fan Verification" options={VERIFICATION_OPTIONS}
                                    ...
                                    helpText="Proof of identity at sign-up. ID checks stop fake accounts but frustrate fans — use only when bot pressure is high." />
                            </div>
                        </ConsolePanel>
                        </div>

                        <div data-tour="config-purchase">
                        <ConsolePanel title="Purchase Rules" icon="Ticket">
                            <div className="space-y-5">
                                <GameSlider label="Purchase Limit" value={config.purchaseLimit}
                                    ...
                                    helpText="Max tickets per buyer. Lower spreads seats across more fans (fairer); higher lets groups — and bots — grab blocks." />
                                <SegmentedControl label="Resale Policy" options={RESALE_OPTIONS} columns={4}
                                    ...
                                    helpText="Rules for reselling tickets. Tighter policies curb scalping and lift fairness where resale pressure is high; too strict can annoy fans who need to transfer." />
                            </div>
                        </ConsolePanel>
                        </div>

                        <div data-tour="config-inventory">
                        <ConsolePanel title="Inventory Allocation" icon="Layers">
                            <div className="space-y-5">
                                <GameSlider label="VIP / Presale Reserve" value={config.presalePercent}
                                    ...
                                    helpText="Share of seats held back for presale before the public sale. Eases public-sale load, but over ~30% leaves the public feeling shut out." />
                                <GameSlider label="Accessible Priority" value={config.accessiblePercent}
                                    ...
                                    helpText="Seats reserved for accessible access. Higher coverage lifts fairness; very low coverage drags it down." />
                            </div>
                        </ConsolePanel>
                        </div>
                    </div>
                    ...
                    <div className="hidden lg:block space-y-3">
                        <div style={{ position: 'sticky', top: '64px' }}>
                            <div data-tour="config-projections">
                            ...
                            <button
                                onClick={launch}
                                data-tour="config-launch"
                                className={`qq-press w-full mt-3 flex items-center justify-center gap-2 px-4 py-4 bg-green-500 hover:bg-green-400 font-bold text-base rounded transition-all glow-green ${dangerCount > 0 ? 'animate-launch-intense' : ''}`}
                                style={{ color: '#0a0e14' }}
                            >
                            ...
                            <p className="text-[10px] text-slate-500 text-center mt-2">
                                {dangerCount > 0 ? `${dangerCount} critical alert${dangerCount > 1 ? 's' : ''} active — launch anyway?` : 'Simulation runs with current settings'}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-terminal-bg border-t-2 border-cyan-500/30">
                ...
                <div className="p-3 flex gap-2">
                    <button
                        ...
                        data-tour="config-projections-mobile"
                        className="qq-press flex items-center justify-center gap-1.5 px-3 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-sm rounded border border-slate-700"
                    >
                    ...
                    <button
                        onClick={launch}
                        data-tour="config-launch-mobile"
                        className={`qq-press flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-400 font-bold text-sm rounded glow-green ${dangerCount > 0 ? 'animate-launch-intense' : ''}`}
                        style={{ color: '#0a0e14' }}
                    >
            ...
            <ScreenTour tourId="configuration" steps={CONFIGURATION_TOUR} helpBottomPx={88} />
        </div>
    );
}
